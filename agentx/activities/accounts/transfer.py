...
from agentx.foundation import BaseSolanaClient
from agentx.constants import LAMPORTS_PER_SOL
from agentx.types import TransferResult
...
class SolanaTransferHelper:
    ...
    @staticmethod
    async def transfer_native_sol(agent: BaseSolanaClient, to: Pubkey, amount: float) -> str:
        """
        Transfer native SOL.

        Args:
            agent: BaseSolanaClient instance
    ...
    @staticmethod
    async def confirm_transaction(agent: BaseSolanaClient, signature: str) -> None:
...
class TokenTransferManager:
    ...
    def __init__(self, agent: BaseSolanaClient):
        self.agent = agent
